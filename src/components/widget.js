...
function listenClicks(product) {
  $(`#${product.id}`).click(() => {
    ...
    ** Remember to make the requests when enter in the page
    ** in the next access.
    */
...
function listenImpression(widget) {
  // Check if the widget exists in the page.
  if ($(`#${widget.id}`).length) {
    // Set impressionStatus to request only once for widget when viewed.
    ...
    */
    $(window).scroll(() => {
      ...
      if (isInViewport(document.getElementById(widget.id)) && widget.impressionStatus === false) {
        // Set impressionStatus to avoid unnecessary/wrong requests.
...
export const Widget = {
  ...
  */
  listenEvents(widget) {
    ...
    // For each product is necessary set the Click track listening.
